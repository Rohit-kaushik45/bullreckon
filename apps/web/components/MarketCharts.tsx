...
const MarketChart = memo(function MarketChart({
  ...
  useEffect(() => {
    ...
    if (containerRef.current) {
      ...
      // Remove any existing charts from DOM
      const existingCharts = document.querySelectorAll("[data-chart-instance]");
      existingCharts.forEach((chart) => {
        const chartInstanceId = chart.getAttribute("data-chart-instance");
        if (chartInstanceId !== instanceId && chart.parentNode) {
          console.log(`Removing duplicate chart instance: ${chartInstanceId}`);
  ...
  // Only render if this is the active chart instance
  if (globalActiveChartId && globalActiveChartId !== instanceId) {
...
const areEqual = (prevProps: MarketChartProps, nextProps: MarketChartProps) => {
  ...
  // Compare historical data length and symbol
  if (
    prevProps.historical.data?.data?.length !==
      nextProps.historical.data?.data?.length ||
    prevProps.historical.data?.symbol !== nextProps.historical.data?.symbol
  ) {
    return false;
  }
