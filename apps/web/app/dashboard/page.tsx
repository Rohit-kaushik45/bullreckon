...
import { TrendingUp, TrendingDown, Wallet, PieChart, Trophy, DollarSign } from "lucide-react";
...
const DashboardPage = () => {
  ...
  const totalPnL = portfolio.positions.reduce((sum, pos) => sum + pos.unrealized_pnl, 0);
      ...
      <main className="lg:pl-64">
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={isProfitable ? "default" : "destructive"} className="px-3 py-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="trading-gradient">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {isProfitable ? "+" : ""}{((totalPnL / portfolio.total_value) * 100).toFixed(2)}%
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="trading-gradient">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Across {new Set(portfolio.positions.map(p => p.symbol.replace('USDT', ''))).size} assets
                </p>
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={equityCurveData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(222, 16%, 25%)" />
              ...
              <CardContent>
                <div className="space-y-4">
                  {portfolio.positions.map((position) => (
                    <div key={position.symbol} className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <div>
                        <div className="font-medium">{position.symbol}</div>
                        <div className="text-sm text-muted-foreground">
                          {position.quantity} @ ${position.avg_buy_price.toLocaleString()}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">${(position.quantity * position.current_price).toLocaleString()}</div>
                        <div className={`text-sm ${position.unrealized_pnl >= 0 ? "text-success" : "text-destructive"}`}>
                          {position.unrealized_pnl >= 0 ? "+" : ""}${position.unrealized_pnl.toLocaleString()} 
                          ({position.unrealized_pnl >= 0 ? "+" : ""}{position.unrealized_pnl_percentage.toFixed(2)}%)
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {mockLeaderboard.slice(0, 5).map((entry) => (
                    <div key={entry.rank} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="font-medium text-success">+{entry.roi}%</div>
                        <div className="text-sm text-muted-foreground">${entry.pnl}</div>
                      </div>
                    </div>
                  ))}
                </div>
