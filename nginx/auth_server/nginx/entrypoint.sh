...
# Determine how many instances to read. If AUTH_BACKEND_COUNT is set use that, otherwise scan up to 20
COUNT=${AUTH_BACKEND_COUNT:-1}
i=1
while [ "$i" -le "$COUNT" ]; do
  VAR_NAME="AUTH_BACKEND_URL_$i"
...
# If no numbered instances found, fall back to single AUTH_BACKEND_URL
if [ -z "$UPSTREAM_BLOCK" ] && [ -n "$AUTH_BACKEND_URL" ]; then
  UPSTREAM_BLOCK="        server $AUTH_BACKEND_URL;"
  echo "Using single backend: $AUTH_BACKEND_URL"
fi
...
# Generate nginx.conf from template with least_conn and origin validation
cat > "$TARGET_CONF" <<EOF
user  nginx;
