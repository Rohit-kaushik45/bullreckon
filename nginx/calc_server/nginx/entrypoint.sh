...
# Determine how many instances to read. If CALC_BACKEND_COUNT is set use that, otherwise scan up to 20
COUNT=${CALC_BACKEND_COUNT:-1}
i=1
while [ "$i" -le "$COUNT" ]; do
  VAR_NAME="CALC_BACKEND_URL_$i"
...
# If no numbered instances found, fall back to single CALC_BACKEND_URL
if [ -z "$UPSTREAM_BLOCK" ] && [ -n "$CALC_BACKEND_URL" ]; then
  UPSTREAM_BLOCK="        server $CALC_BACKEND_URL;"
  echo "Using single backend: $CALC_BACKEND_URL"
fi
...
$(printf '%b' "$UPSTREAM_BLOCK")
    ...
    server {
        ...
        access_log  /var/log/nginx/calc_access.log;
        error_log   /var/log/nginx/calc_error.log;
    }
}
EOF
