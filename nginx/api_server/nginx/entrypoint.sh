...
# Determine how many instances to read. If API_BACKEND_COUNT is set use that, otherwise scan up to 20
COUNT=${API_BACKEND_COUNT:-1}
i=1
while [ "$i" -le "$COUNT" ]; do
  VAR_NAME="API_BACKEND_URL_$i"
...
# If no numbered instances found, fall back to single API_BACKEND_URL
if [ -z "$UPSTREAM_BLOCK" ] && [ -n "$API_BACKEND_URL" ]; then
  UPSTREAM_BLOCK="        server $API_BACKEND_URL;"
  echo "Using single backend: $API_BACKEND_URL"
fi
