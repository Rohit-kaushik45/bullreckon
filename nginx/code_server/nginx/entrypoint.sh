...
# Determine how many instances to read. If CODE_BACKEND_COUNT is set use that, otherwise scan up to 20
COUNT=${CODE_BACKEND_COUNT:-1}
i=1
while [ "$i" -le "$COUNT" ]; do
  VAR_NAME="CODE_BACKEND_URL_$i"
...
# If no numbered instances found, fall back to single CODE_BACKEND_URL
if [ -z "$UPSTREAM_BLOCK" ] && [ -n "$CODE_BACKEND_URL" ]; then
  UPSTREAM_BLOCK="        server $CODE_BACKEND_URL;"
  echo "Using single backend: $CODE_BACKEND_URL"
fi
