...
const tradeSchema = new mongoose.Schema(
  {
    ...
    source: {
      ...
      enum: {
        values: ["market", "limit", "strategy", "stop_loss", "take_profit"],
...
tradeSchema.pre("save", function (next) {
  if (!this.fees) {
    this.fees = (this as unknown as ITrade).calculateFees(
      this.quantity * this.triggerPrice
    );
  }
  ...
  if (
    !this.marketData ||
    typeof this.marketData.high !== "number" ||
    typeof this.marketData.low !== "number" ||
    typeof this.marketData.open !== "number" ||
    typeof this.marketData.close !== "number"
  ) {
    return next(new Error("Market data is missing or invalid"));
  }
